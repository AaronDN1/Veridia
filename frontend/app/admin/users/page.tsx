  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const [adminUsers, adminFeedback] = await Promise.all([getAdminUsers(), getAdminFeedback()]);
        setUsers(adminUsers);
        setFeedbackItems(adminFeedback);
        setSelectedFeedbackId(adminFeedback[0]?.id ?? null);
      } catch (err) {
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <p className="mt-3 text-sm text-slate-500 dark:text-slate-300">
                      {selectedFeedback.submitter_email ?? "Anonymous"} · {formatDate(selectedFeedback.created_at)}
                    </p>
                  </div>
                </div>
